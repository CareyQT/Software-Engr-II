import { Course, CourseRecord, CourseSearchFilters } from '@/features/courses/interfaces/course'
import { COURSE_CATALOG } from '@/features/courses/lib/course-catalog'
import { normalizeCourseCode } from '@/features/courses/utils/normalize-course-code'
import { parseTermSeason } from '@/interfaces/academic-term'
import pool from '@/lib/postgres'

export async function getAllCourses() {
  try {
    const result = await pool.query('SELECT * FROM Course ORDER BY code')
    return result.rows as CourseRecord[]
  } catch (error) {
...
export async function getCourseById(id: number) {
  try {
    ...
    return (result.rows[0] as CourseRecord) ?? null
  } catch (error) {
...
export async function createCourse(
  ...
  try {
    ...
    return result.rows[0] as CourseRecord
  } catch (error) {
...
export function searchCourses(filters: CourseSearchFilters): { courses: Course[]; total: number } {
  try {
    ...
    const department = filters.department?.trim().toUpperCase() ?? ''
    ...
    const courses = COURSE_CATALOG.filter(course => {
      ...
      if (department && course.subject !== department) {
        return false
      }
