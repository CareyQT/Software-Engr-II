...
export function TermwisePlanner() {
  ...
  }, [courseResults, departmentFilter])
  ...
  useEffect(() => {
    ...
    const currentExists = filteredCourseResults.some(course => course.code === selectedCourseCode)
    if (!currentExists) {
      setSelectedCourseCode(filteredCourseResults[0].code)
    }
  ...
  function addTerm() {
    setTerms(previous => {
      const nextTerm = createNextTerm(previous)
      return [...previous, nextTerm]
    })
  }
        ...
        <aside className="flex w-[92px] flex-col border-r border-zinc-200 bg-[#fbfbfb]">
          ...
          <div className="mt-auto border-t border-zinc-200 px-2 py-3">
            ...
            <div className="mt-3 rounded-md border border-zinc-200 bg-white px-2 py-2 text-[10px] text-zinc-600">
              <p className="font-medium text-zinc-800">Eduardo</p>
              <p>huzhnerb@oregonstate.edu</p>
            </div>
          </div>
        </aside>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Select
                        ...
                        onValueChange={value =>
                          setDepartmentFilter(
                            value === ALL_DEPARTMENTS_FILTER ? '' : value.toUpperCase()
                          )
                        }>
                    ...
                    <div className="max-h-[56vh] space-y-2 overflow-auto border-t border-zinc-200 pt-2">
                      ...
                      {!isLoadingCourses && filteredCourseResults.length === 0 && (
...
function createNextTerm(terms: PlannedTerm[]): PlannedTerm {
